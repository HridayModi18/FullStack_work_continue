...
const TextPostForm = ({ onSubmit, isSubmitting }) => {
  ...
  return (
    <form className="post-form" onSubmit={handleSubmit}>
      ...
      >
        ...
        {isSubmitting ? "Uploading..." : "Publish Text Post"}
      </button>
    </form>
  );
};

const PollPostForm = ({ onSubmit, isSubmitting }) => {
  ...
  return (
    <form className="post-form" onSubmit={handleSubmit}>
      ...
      >
        ...
        {isSubmitting ? "Uploading..." : "Publish Poll"}
      </button>
    </form>
  );
};

const MediaUploadForm = ({ type, onSubmit, isSubmitting }) => {
  const [title, setTitle] = useState("");
  const [content, setContent] = useState("");
  const [tags, setTags] = useState([]);
    ...
    <form className="post-form" onSubmit={handleSubmit}>
      ...
      {type === "Assignment" && (
        <>
          ...
          <div className="form-group">
            <label>Assignment Number (1-20) *</label>
            <input
              type="number"
              min="1"
              max="20"
      ...
      >
        ...
        {isSubmitting ? "Uploading..." : `Publish ${type}`}
      </button>
    </form>
  );
};

const CreatePostModal = ({ type, onClose }) => {
  const [isSubmitting, setIsSubmitting] = useState(false);

  const handleOverlayClick = (e) => {
    if (e.target.classList.contains("modal-overlay") && !isSubmitting) {
      onClose();
    }
  };
  ...
  const submitToBackend = async (postData) => {
    ...
    try {
      ...
      // jwt token attach karo for authentication
      const authHeaders = {
      ...
      //baaki saare formdata payload pe
      else {
      ...
      alert(`Success! Your ${type} has been posted to the bootcamp`);
      onClose();
    } catch (err) {
      console.error(err);
      alert(`Failed to pst: ${err.response?.data?.message || err.message}`);
    } finally {
    ...
    <div className="modal-overlay" onClick={handleOverlayClick}>
      <div className="modal-container">
        ...
        <div className="modal-header">
          <h2>Create {type}</h2>
          <p>Fill out the details below to post to the bootcamp.</p>
        </div>
